...
El servicio Windows `GnxAppMonitor` reconcilia una cadena mínima y explícita:

```text
Servicio Windows (LocalSystem)
  → usuario local dedicado gnxsvc
  → WSL2 + Ubuntu-24.04
  → systemd de WSL
  → Podman Quadlet
  → gnx-linux.service generado por Quadlet
```
...
|---|---|---|---|---|
| WSL-001 | Crear una identidad Windows dedicada | Crea `gnxsvc` solo si no existe; bloquea conflictos de ownership | Implementado | `DEDICATED_USER_CONFLICT` o `userReady=true` en estado |
| WSL-002 | Proteger la credencial de la cuenta gestionada | Guarda una credencial DPAPI en `ProgramData`, ACL para SYSTEM/Administrators | Implementado | `gnxsvc.credential.xml`; nunca se escribe el secreto en logs |
...
| WSL-005 | Instalar Ubuntu 24.04 bajo la identidad dedicada | Usa `wsl --install --distribution Ubuntu-24.04 --no-launch` como `gnxsvc` | Implementado | `WSL_INSTALL_FAILED` o distribución registrada |
...
| WSL-007 | Adoptar Podman Quadlets | Instala Podman y escribe `/etc/containers/systemd/gnx-linux.container` | Implementado | Quadlet fuente gestionado por el reconciliador |
| WSL-008 | Servicio Linux resiliente | systemd genera `gnx-linux.service`; Quadlet usa `Restart=always`, 5 s de backoff, rootfs read-only y red aislada | Implementado | `systemctl is-active gnx-linux.service` |
...
| `DEDICATED_USER_CONFLICT` | `gnxsvc` existe pero no es propiedad de GnX | Resolver con un administrador; nunca adoptar la cuenta a ciegas. |
...
wsl -d Ubuntu-24.04 -u root -- systemctl cat gnx-linux.service
```
